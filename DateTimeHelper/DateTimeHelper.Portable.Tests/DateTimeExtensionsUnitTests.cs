using System;
using DateTimeHelper.Portable;
using Microsoft.VisualStudio.TestTools.UnitTesting;

namespace DateTimeHelper.Portable.Tests
{
    [TestClass]
    public class DateTimeExtensionsUnitTests
    {
        ...
        {
            var result = dateTime.GetFirstDayOfWeek(DayOfWeek.Monday);
            var expectedDate = new DateTime(2017, 10, 09, 00, 00, 00);
            Assert.AreEqual(0, expectedDate.CompareTo(result));
        }

        [TestMethod]
        public void GetFirstDayOfWeekTest2()
        {
            var result = dateTime.GetFirstDayOfWeek(DayOfWeek.Sunday);
